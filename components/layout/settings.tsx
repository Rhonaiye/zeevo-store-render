...
import { Loader2, User, Mail, Camera, Crown, ArrowUpRight, Check, X, CreditCard, Ban as Bank, Plus, Trash2, ChevronDown, Info, CheckCircle, XCircle } from 'lucide-react';
...
const RenderSettings: FC<RenderSettingsProps> = ({ isLoading, addNotification }) => {
  ...
  // Handle KYC NIN verification
  const handleVerifyNIN = async () => {
    if (!kycForm.nin) {
      if (addNotification) {
        addNotification('Please enter your NIN', 'error');
      }
      return;
    }
    ...
    try {
      ...
      setKycResult(data);
      if (addNotification) {
        addNotification('KYC verification successful', 'success');
      }
      setShowKycForm(false);
      setKycForm({ nin: '' });
    } catch (err: any) {
      const errorMsg = err.message || 'Verification failed';
      setKycResult({ error: errorMsg });
      if (addNotification) {
  ...
  return (
    <div className="relative bg-white rounded-xl sm:rounded-2xl border border-gray-100 p-4 sm:p-6">
      {/* Fullscreen Loader */}
      {isVerifying && (
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          {/* Identity & Financial Verification Group */}
          <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600">Verify your identity to start collecting payments</p>
                  </div>
                </div>
                {!hasKYC && (
                  <motion.button
                  ...
                  >
                    ...
                    <span className="sm:inline">Verify KYC</span>
                  </motion.button>
                )}
              </div>
              ...
              {hasKYC ? (
                <div className="bg-white p-3 rounded-lg border border-green-200">
                  <div className="flex items-center gap-2 text-sm text-green-700">
                    <CheckCircle className="w-4 h-4" />
                    <span>KYC Verified - You can now collect payments</span>
                  </div>
                </div>
              ) : (
                showKycForm && (
                  ...
                  >
                    ...
                    <form className="space-y-4">
                      <div>
                        <label className="block text-xs font-medium text-gray-800 mb-2">NIN Number</label>
                        <input
                          type="text"
                          name="nin"
                          placeholder="Enter your 11-digit NIN"
                          value={kycForm.nin}
                          onChange={(e) => setKycForm({ ...kycForm, [e.target.name]: e.target.value })}
                          maxLength={11}
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        >
                          ...
                          {kycLoading ? 'Verifying...' : 'Verify & Continue'}
                        </motion.button>
                        <button
                          type="button"
                          onClick={() => {
                      ...
                      >
                        {kycResult.error ? (
                          ...
                          <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                            <p className="text-xs text-green-800 font-medium mb-2 flex items-center gap-2">
